...
class GetPath(State):
    ...
    def execute(self, userdata):
        ...
        while not self.path_ready:
            try:
                pose = rospy.wait_for_message(topic, PoseWithCovarianceStamped, timeout=5)#timeout=1
...
class PathComplete(State):
    ...
    def execute(self, userdata):
        end_command = Empty()
        pub_end_path = rospy.Publisher('/finish_path', Empty, queue_size=1)
        pub_end_path.publish(end_command)
        rospy.loginfo('###############################')
